...
M.event = "FileType"
M.pattern = "typescript"

M.cond_func = function()
    local cwd = vim.fn.getcwd()

    if vim.fs.basename(cwd) == "client" then
        return true
    end

    local child_check = vim.fn.isdirectory(vim.fs.joinpath(cwd, "client")) == 1
    local svn_check = vim.fn.isdirectory(vim.fs.joinpath(cwd, ".svn")) == 1

    return child_check and svn_check
end
        ...
        ["init label-view"] = {
            ...
            content = function(prefab_path)
                ...
                return {
                    ...
                    "            const isOpen = typeof baseInfo.checkOpenFunc === 'function' ? baseInfo.checkOpenFunc().ok : true;",
                    "",
                    "            if (isOpen) {",
                    "                this._labelInfoDict.set(baseInfo.labelType!, {",
                    "                    ...baseInfo,",
